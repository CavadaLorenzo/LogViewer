...
class myThread(threading.Thread):
    ...
    def run(self):
        self.fileLog = open("./LogViewer.log", "a")
        print("Reading from: " + self.name)
        self.fileLog.write("READING FROM FILE " + self.name + "\n")
        self.file.seek(0, 2)
        ...
        while True:
            ...
            if not line:
                ...
                oldRequest = self.parseNewLine(line, oldRequest)

    def updateDatabase(self, req):
    ...
    def parseNewLine(self, line, oldRequest):
        line = line.split(" ")
        if line[len(line) - 1] == "c\n":
            ...
            if request.reqJson["filePath"] != oldRequest.reqJson["filePath"] or request.reqJson["date"] > (
                ...
                self.fileLog.write("PARSED REQUEST RECEIVED AT: " + self.name + "\n" + str(request))
                self.updateDatabase(request)
                oldRequest = request
        return oldRequest
